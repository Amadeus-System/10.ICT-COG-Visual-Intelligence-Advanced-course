...
capture = cv2.VideoCapture('capture_.avi')
# 동영상을 저장할 때 지정한 FPS
...
# 다음과 같은 범위에 들어가는 이미지의 픽셀만 추출 - 얼굴의 경우 Cr 범위가 중요.
lower_range = np.array([max(Ymean-60,0), max(Umean-10,0), max(Vmean-20,0)])
upper_range = np.array([min(Ymean+60,255), min(Umean+10,255), min(Vmean+20,255)])
