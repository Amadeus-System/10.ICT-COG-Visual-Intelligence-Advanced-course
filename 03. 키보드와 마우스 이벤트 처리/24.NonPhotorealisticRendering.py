...
# - 사실적 렌더링 : 트랜스포머나 캐리비안의해적과 같이 실제 영상과 구분이 되지 않는 영상을 생성 방법
...
# - NPR은 회화나 드로잉, 도해, 만화 같은 인공적인 양식에 영향을 받는다.
# - 예로 만화 같은 그림을 묘사하는 비디오 게임이나 영화에서 사용하는 툰 셰이딩이 있다.
...
# - smoothing을 하면서도 edge를 보존하는 필터로 biliteral와 유사하지만 훨씬 빠르다.
# - flags: 가장자리 보존 필터를 지정. RECURS_FILTER=1, NORMCONV_FILTER =2
# -- RECURS_FILTER가 3.5배 빠르나, stylizations 용도로는 sharpening이 좋은 2를 지정.
# - sigma_s(space) : [0, 200]. 가우시안 Var로서 인접 픽셀과 가중평균할 kernel size를 결정.
# - sigma_r(range) : (0, 1]. 유사 색상 픽셀의 범위를 지정하는 가우시안 Var
def on_ep_change(_):
...
# cv2.stylization(src, sigma_s, sigma_r)
# - 애니메이션과 같은 효과를 준다.
def on_st_change(_):
