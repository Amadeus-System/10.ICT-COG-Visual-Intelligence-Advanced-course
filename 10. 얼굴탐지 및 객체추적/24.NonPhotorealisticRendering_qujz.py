...
def on_epps_change(_):
	...
	_, dst = cv.pencilSketch(dst, sigma_s=sigma_s, sigma_r=sigma_r)
	cv.imshow('edge_p and pencil_sk', dst)
...
def on_deps_change(_):
	...
	print('pencil_sketch: sigma_s, sigma_r =', sigma_s, sigma_r)
	_, dst = cv.pencilSketch(dst, sigma_s=sigma_s, sigma_r=sigma_r)
	cv.imshow('detail_e and pencil_sk', dst)

# 비사실적 효과의 순서를 바꿀 때, 어떠한 변화가 있는 지 관찰하기 위한 함수
def on_psde_change(_):
	sigma_s = cv.getTrackbarPos('sigma_s_ps', 'pencil_sk and detail_e')
	sigma_r = cv.getTrackbarPos('sigma_r_ps', 'pencil_sk and detail_e') * 0.01
	print('pencil_sketch: sigma_s, sigma_r =', sigma_s, sigma_r)
	_, dst = cv.pencilSketch(img, sigma_s=sigma_s, sigma_r=sigma_r)
	sigma_s = cv.getTrackbarPos('sigma_s_de', 'pencil_sk and detail_e')
	sigma_r = cv.getTrackbarPos('sigma_r_de', 'pencil_sk and detail_e') * 0.01
	print('detail_enhanced: sigma_s, sigma_r =', sigma_s, sigma_r)
	dst = cv.detailEnhance(dst, sigma_s=sigma_s, sigma_r=sigma_r)
	cv.imshow('pencil_sk and detail_e', dst)
...
# 비사실적 효과의 순서를 바꿀 때, 어떠한 변화가 있는 지 관찰하기 위한 윈도우
cv.namedWindow('pencil_sk and detail_e')
cv.createTrackbar('sigma_s_de', 'pencil_sk and detail_e', 10, 60, on_psde_change)
cv.createTrackbar('sigma_r_de', 'pencil_sk and detail_e', 15, 100, on_psde_change)
cv.createTrackbar('sigma_s_ps', 'pencil_sk and detail_e', 60, 60, on_psde_change)
cv.createTrackbar('sigma_r_ps', 'pencil_sk and detail_e', 20, 100, on_psde_change)
...
# edge_p and detail_e: 그런대로 준수한 편
# edge_p and pencil_sk: pensil sketch 효과만
# edge_p and style: 깔끔한 애니효과
# detail_e and pencil_sk: pensil sketch 효과만
# detail_e and style: 지저분
# pencil_sk and style: 기괴함 - 순서를 바꿀 때, 효과가 다를 것이다.